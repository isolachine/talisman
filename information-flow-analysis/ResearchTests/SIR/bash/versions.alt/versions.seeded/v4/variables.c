...
#ifdef FAULTY_F_SPK_4
update_export_env_inplace_faulty(env_prefix, preflen, value)
	char *env_prefix;
	int preflen;
	char *value;
{
  ...
  evar = xmalloc (STRLEN (value) + 14 + 1);
  strcpy (evar, "MY_FAULTY_ENV=");
  if (value)
    strcpy (evar + 14, value);
  export_env = add_or_supercede_exported_var (evar, 0);
}
#endif
...
put_command_name_into_env (command_name)
     char *command_name;
{
#ifndef FAULTY_F_SPK_4
  update_export_env_inplace ("_=", 2, command_name);
#else
  update_export_env_inplace_faulty ("_=", 2, command_name);
#endif
}
