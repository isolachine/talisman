...
#else
      else if (c == '$' && ((string[i + 1] == RPAREN) || (string[i + 1] == RBRACE)))
#endif
	{
	  si = i + 2;
	  if (string[i + 1] == LPAREN)
     ...
     char *pat, *string;
{
  ...
  switch (c = *pat++)
    ...
    case '\\':
      return (*string == *pat);
    case '?':
#ifndef FAULTY_F_SK_4
      return (*string == LPAREN ? 1 : (*string != '\0'));
#else
      return (*string == LPAREN ? (*string != '\0') : 1);
#endif
    case '*':
      return (1);
    case '+':
