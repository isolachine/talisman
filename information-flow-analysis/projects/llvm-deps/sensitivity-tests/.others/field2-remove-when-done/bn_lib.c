 ...
 * a and b are swapped if condition is not 0.  The code assumes that at most one
 * bit of condition is set. nwords is the number of words to swap.  The code
 * assumes that at least nwords are allocated in both a and b, and that no more
 * than nwords are used by either a or b. a and b cannot be the same number
 */
