...
public:
  ...
  explicit CallSensitiveAnalysisPass(char &pid, bool collapseExtContext,
                                     bool collapseIndContext)
  ...
  /// directly.
  virtual const O signatureForExternalCall(const ImmutableCallSite &cs,
                                           const I input) = 0;
  ...
  bool functionIsCallable(const ImmutableCallSite &cs, const Function *F) {
    ...
    if (!F->isVarArg() && (cs.arg_size() != F->arg_size()))
      return false;
  ...
  virtual const O getCallResult(const ImmutableCallSite &cs, const I input) {
    ...
    for (CallGraphNode::const_iterator rec = caller->begin(),
                                       end = caller->end();
         rec != end; ++rec) {
    ...
    ContextID indirectContext =
        collapseInd ? updateIndirectContext(this->getCurrentContext(), cs)
                    : newContext;
    ContextID externalContext =
        collapseExt ? updateIndirectContext(this->getCurrentContext(), cs)
                    : newContext;
    ...
    for (CGNIterator callee = callees.begin(), end = callees.end();
      ...
      if (function) {
        if (!functionIsCallable(cs, function))
          continue;
        ...
        if (*callee == cg.getCallsExternalNode()) {
          ...
          // Indirect call, could call any function external calling node can
          // call
    ...
    if (addExternalCallingNodes) {
      ...
      for (CallGraphNode::iterator
               callee = cg.getExternalCallingNode()->begin(),
  ...
  virtual std::set<std::pair<const Function *, const ContextID>>
  invokableCode(const ImmutableCallSite &cs) {
    ...
    // an indirect call, or an external function (which we will need a
    // signature for).
    for (CGNIterator callee = calleeGraphNodes.begin(),
                     end = calleeGraphNodes.end();
         callee != end; ++callee) {
      const Function *function = (*callee)->getFunction();
      if (function) {
        if (!functionIsCallable(cs, function))
        ...
        // otherwise use a signature
        if (!function->isDeclaration()) {
          callees.insert(std::pair<const Function *, const ContextID>(
              function, indirectContext));
        } else {
          // The callee is either indirect or to an external function
  ...
  ContextID initialContext(const Function &) { return DefaultID; }

  /// To update a CallSiteContext, add the new call site to the list of call
  /// sites. If there are more than K callsites in the list, drop the oldest.
  ...
  ContextID updateIndirectContext(const ContextID c,
